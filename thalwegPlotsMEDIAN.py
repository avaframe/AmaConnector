...
def plotBoxPlot(dbData, colList, outDir, namePlot, ylim=(0,0), split='', renameCols=[], renameTitle=[], renameY=[], renameX=[]):
    ...
    if split !='':
        ...
        for i, col in enumerate(colList):
            ...
            if len(colList) == 1:
                sn = sns.violinplot(data=dbData[col], color= '#a3a3a3', fill=False, cut=0)
                sn = sns.stripplot(data=dbData[col], jitter=True, color ='grey', size=2,  zorder=1, alpha =0.45)
                sn = sns.violinplot(data=dbData, x=split, y=col, palette= colPalette, fill=False, cut=0)
                sns.stripplot(data=dbData, x=split, y=col, jitter=True, color ='grey', size=2,  zorder=1, alpha=0.45)
                ...
                for xtick, (mean, quantil) in enumerate(zip(means, quantile)):
                    ...
                    axes.text(xtick, mean, f'{mean:.1f}', ha='center', va='top', color='black', size=18)
                ...
                for xtick, group in enumerate(samplesize.index):
                    ...
                    axes.text(xtick, ylim[0], f'n={n}', ha='center', va='bottom', color='black', fontsize=15)
                    
    
                if renameTitle == []:
                    axes.set_title('Distribution of Maxpotsize for %s'%col[0:20], loc='left')
                else:
                ...
                sn = sns.violinplot(data=dbData[col], ax=axes[i], color= '#a3a3a3', fill=False, cut=0)
                sn = sns.stripplot(data=dbData[col], ax=axes[i], jitter=True, color ='grey', size=2,  zorder=1, alpha =0.45)
                sn = sns.violinplot(data=dbData, x=split, y=col, ax=axes[i], palette= colPalette, fill=False, cut=0)
                sns.stripplot(data=dbData, x=split, y=col, ax=axes[i], jitter=True, color ='grey', size=2,  zorder=1, alpha =0.45)
                #sns.set(style="darkgrid")
                ...
                for xtick, mean in enumerate(means):
                    #space = quantil-mean
                    #axes[i].text(xtick, mean, f'{mean:.2f}\n{quantil:.2f}', ha='center', va='top', color='black', size=18)
                    axes[i].text(xtick, mean, f'{mean:.1f}', ha='center', va='top', color='black', size=18)
                ...
                for xtick, group in enumerate(samplesize.index):
                    ...
                    axes[i].text(xtick, ylim[0], f'n={n}', ha='center', va='bottom', color='black', size=12)
                                    
                #axes[i].set_xlabel( split )
                axes[i].set_ylabel(namePlot[:-1])
                if renameTitle == []:
                    axes[i].set_title('Distribution of Maxpotsize for %s'%col[0:20], loc='left')
                else:
                    #axes[i].set_title('')
        ...
        colPalette = ['#a1cae2', '#0b3f5e', '#3b93ea','#de6326','#d17547','#0fbddb', '#1c5761']
        fig = plt.figure(figsize =(6*int(len(colList)),8))
        ax3 = plt.subplot(111)
        sns.stripplot(data=dbData[colList], jitter=True, color ='grey', alpha=0.5, size=2,  zorder=1)
        ...
        for idx, col in enumerate(colList):
            ...
            x_position = idx 
            ax3.text(x_position, mean_value, f'{mean_value:.1f}', ha='center', va='center', color='black', fontsize=22)
            
        if renameTitle == []:
            ax3.set_title('Distribution of %s for %d events' % (namePlot, len(dbData[colList[0]])), fontsize=20)
        else:
            ax3.set_title(renameTitle[0] + ' for %d events' %len(dbData[colList[0]]),fontsize=20)
        ...
        if pathList != []:
            for path in pathList:
                if path == 'curveFit_s_z':
                    ...
                    plt.plot(row[path].xy[0], row[path].xy[1],  label=label, color = 'blue', alpha = 1, lw=1)
                else:
        ...
        for  point in pointList:
            ...
            if pd.notna(row[point]):
                pointDict = {'x':[row[point].x], 
                             'y':[row[point].y]}
                pointAvapath = gT.findClosestPoint(avaPath['x'], avaPath['y'], pointDict)
                
                if 'rel' in point:
                    legend = 'release point'
                    #legend = 'depo γ(depo): ' +str(round(row['orig-depo_Angle'],2)) 
                    plt.plot(avaPath['s'][pointAvapath], avaPath['z'][pointAvapath], '*', markersize=10, color='lightgrey', label=legend)
                                      
                if 'orig' in point:
                    legend = 'O-point'
                    label = round(row['geom_origin_pt3d_epsg:31287_snapped_gradient'],2)
                    plt.plot(avaPath['s'][pointAvapath], avaPath['z'][pointAvapath], 'c*', markersize=10, label=legend)
                    #plt.text(avaPath['s'][pointAvapath], avaPath['z'][pointAvapath], label, fontsize=7, va='top', ha='right')
                    
                if 'transit' in point:
                    legend = 'T-point $\gamma_T:$' + str(round(row['orig-transit_Angle'],2))
                    #legend = 'transit point'
                    label = round(row['geom_transit_pt3d_epsg:31287_snapped_gradient'],2)
                    plt.plot(avaPath['s'][pointAvapath], avaPath['z'][pointAvapath], 'g*', markersize=10,  label=legend)
                    #plt.text(avaPath['s'][pointAvapath], avaPath['z'][pointAvapath], label, fontsize=7, va='top', ha='right')  
                    
                if 'runout' in point:
                    label = round(row['geom_runout_pt3d_epsg:31287_snapped_gradient'],2)
                    #legend = 'deposition point'
                    legend = r'D-point $\gamma_D =\beta:$' +str(round(row['orig-depo_Angle'],2)) 
                    plt.plot(avaPath['s'][pointAvapath], avaPath['z'][pointAvapath], '*', markersize=10, color='#ad1d22', label=legend)
