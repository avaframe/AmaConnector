...
#bbGeometry =  bbFile['geometry'].iloc[0]

#singleEvent = dbFiltered.loc[dbData['event_id']== 590]
#point_geometry = singleEvent['geom_rel_event_pt'].iloc[0]

# Check if the geometry is within the bounding box polygon
#is_within = point_geometry.within(bbFile['geometry'].iloc[0])

#output_geopackage_path = 'C:/Users/LawineNaturgefahren/MagdalenaTest/testArea1/'+str(i)+'event_ln.gpkg'
#exportData.to_file(output_geopackage_path, driver='GPKG')





 #relEvent.rename(columns={'geom_rel_event_ln_epsg:31287':'geometry'})
 
 #path = gpd.GeoDataFrame(singleEvent['geom_rel_event_ln_epsg:31287'], crs='epsg:31287')
 #path['geometry'] = path['geom_rel_event_ln_epsg:31287']
 
 
 #relEvent ['geometry'] = singleEvent['geom_rel_event_ln_epsg:31287'].apply(
   #  lambda x: shapely.geometry.LineString(list(x.coords)))
 
 #coordinates = relEvent.geometry.apply(lambda geom: list(geom.coords))
 #geometry = singleEvent['geom_rel_event_ln_epsg:31287'].apply(lambda x: shapely.geometry.LineString(list(x.coords)))
 #geometry = relEvent['geom_rel_event_ln_epsg:31287'].apply(lambda line: LineString(line))

 #geometry = [gpd.GeoSeries([(x, y) for x, y in coords]) for coords in coordinates]
 #geo_df = gpd.GeoDataFrame(geometry=geometry, columns=['geometry'], crs='EPSG:31287')
 
'''
 coordinates = relEvent.geometry.apply(lambda geom: list(geom.coords))
 
 geometry = coordinates.apply(lambda coords: LineString(coords))
 geo_df = gpd.GeoDataFrame(geometry=geometry, crs='EPSG:4326')  # CRS for WGS 84

 
 output_geopackage_path = 'C:/Users/LawineNaturgefahren/MagdalenaTest/testArea1/' + str(i) + '_event_ln.gpkg'
 geo_df.to_file(output_geopackage_path, driver='GPKG')

'''
 #exportData = gpd.GeoDataFrame( i, coordinates )
 #test = gpd.GeoDataFrame(coordina['geometry'])
 #outputPath = 'C:/Users/LawineNaturgefahren/MagdalenaTest/testArea1/' + str(i) + '_rel_event_ln.gpkg'
 #exportData.to_file(outputPath, driver='GPKG')
